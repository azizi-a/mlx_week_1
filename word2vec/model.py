...
class SkipGram(torch.nn.Module):
    def __init__(self, vocab_size: int, embedding_dim: int):
        ...
        self.forward_weight_matrix = torch.nn.Linear(
    ...
    def forward(self, x, targets, negatives):
        ...
        pos_loss = torch.log(
            torch.sigmoid(
                torch.sum(emb * self.forward_weight_matrix.weight[targets], dim=1)
            )
        )
        neg_loss = torch.log(
            torch.sigmoid(
                -torch.sum(
                    emb.unsqueeze(1) * self.forward_weight_matrix.weight[negatives],
                    dim=2,
                )
            )
        )
...
def train_model(
    corpus: list,
    words_to_ids: typing.Dict[str, int],
    ids_to_words: typing.Dict[int, str],
    ...
    for epoch in range(epochs):
        ...
        for i in progress_bar:
            ...
            negatives = torch.randint(
                0, vocab_size, (batch_size, config.NUM_NEGATIVE_SAMPLES)
            ).to(device)
